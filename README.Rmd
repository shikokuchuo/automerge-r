...
`automerge` provides R bindings to the [Automerge](https://automerge.org/) Conflict-free Replicated Data Type (CRDT) Rust library via its C FFI. Automerge enables automatic merging of concurrent changes without conflicts, making it ideal for:
...
This package is currently in the initial development phase. Some functionality is yet to be implemented, and any existing functionality is subject to change at any time.
...
Cursors provide stable position tracking across edits, while marks attach metadata to text ranges. These features are essential for collaborative text editing applications.

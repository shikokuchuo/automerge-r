...
am_cursor <- function(doc, obj, position) {
  .Call(C_am_cursor, doc, obj, position)
}
...
#' print(pos)  # 5
am_cursor_position <- function(doc, obj, cursor) {
  .Call(C_am_cursor_position, doc, obj, cursor)
}
...
#' print(marks)
am_mark_create <- function(doc, obj, start, end, name, value,
                           expand = AM_MARK_EXPAND_NONE) {
  invisible(.Call(C_am_mark_create, doc, obj, start, end, name, value, expand))
}
...
am_marks <- function(doc, obj) {
  .Call(C_am_marks, doc, obj)
}
...
#'
#' am_mark_create(doc, text_obj, 0, 5, "bold", TRUE)
#' am_mark_create(doc, text_obj, 2, 7, "underline", TRUE)
#'
...
am_marks_at <- function(doc, obj, position) {
  all_marks <- am_marks(doc, obj)

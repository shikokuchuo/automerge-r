test_that("am_cursor creates and retrieves cursor positions", {
  ...
  pos <- am_cursor_position(doc, text_obj, cursor)
  expect_equal(pos, 5)
})
...
test_that("cursors track positions across text edits", {
  ...
  new_pos <- am_cursor_position(doc, text_obj, cursor)
...
test_that("cursors work at text boundaries", {
  ...
  cursor_start <- am_cursor(doc, text_obj, 0)
  expect_equal(am_cursor_position(doc, text_obj, cursor_start), 0)

  # Cursor at end (position 4, at last character 'o')
  # Note: Cursors must be placed within the text, not after it
  cursor_end <- am_cursor(doc, text_obj, 4)
  expect_equal(am_cursor_position(doc, text_obj, cursor_end), 4)
})
...
test_that("cursor validation rejects invalid inputs", {
  ...
  expect_error(am_cursor(doc, text_obj, "invalid"), "position must be numeric")

  # Non-scalar position
  expect_error(am_cursor(doc, text_obj, c(1, 2)), "position must be a scalar")

  # Negative position
  expect_error(am_cursor(doc, text_obj, -1), "position must be non-negative")

  # Invalid cursor in am_cursor_position
  expect_error(am_cursor_position(doc, text_obj, "not_a_cursor"),
...
test_that("multiple cursors work independently", {
  ...
  cursor1 <- am_cursor(doc, text_obj, 2)
  cursor2 <- am_cursor(doc, text_obj, 6)
  cursor3 <- am_cursor(doc, text_obj, 9)

  # Insert text before all cursors
  am_text_splice(text_obj, 0, 0, "XX")

  # All cursors should move by 2
  expect_equal(am_cursor_position(doc, text_obj, cursor1), 4)
  expect_equal(am_cursor_position(doc, text_obj, cursor2), 8)
  expect_equal(am_cursor_position(doc, text_obj, cursor3), 11)
})
...
test_that("cursors remain valid after text deletion", {
  ...
  cursor <- am_cursor(doc, text_obj, 7)
